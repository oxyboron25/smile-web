...
const Founders = () => {
  ...
  useEffect(() => {
    offset.current = window.innerWidth
  }, [])

  const scroll = (delta: number) => {
    const foundersMenu = document.getElementById('founders')
    if (foundersMenu) {
      foundersMenu.scrollBy({ left: delta, behavior: 'smooth' })
    }
  }
  ...
  return (
    <section className="relative h-fit py-20 lg:py-0">
      ...
      <div
        role="button"
        onClick={() => scroll(-offset.current)}
        className="absolute left-0 w-[5%] h-full bg-[#001612] lg:flex flex-col items-center justify-center text-white z-10 hidden"
      >
        <ArrowLeftCircle height={50} width={50} />
      </div>
      ...
      <div
        role="button"
        onClick={() => scroll(offset.current)}
        className="absolute right-0 w-[5%] h-full bg-[#001612] lg:flex flex-col items-center justify-center text-white hidden z-10"
      >
        <ArrowRightCircle height={50} width={50} />
      </div>
      ...
      <motion.div
        id="founders"
        className="w-screen h-fit overflow-x-scroll overflow-y-hidden scroll-smooth"
      >
        <div className="lg:hidden flex flex-col text-white justify-center pb-10 gap-2 items-center">
          <p className="text-6xl text-center font-light">
            Meet the <span className="text-[#5DA399] font-bold">Founders</span>
        ...
        <motion.div className="w-fit flex flex-col lg:flex-row h-fit text-white gap-20 lg:gap-0 snap-mandatory snap-center">
          {founders.map((founder, i) => (
            <FounderElement i={i} key={founder.name} />
          ))}
        </motion.div>
      </motion.div>
    </section>
...
const FounderElement: React.FC<FounderElementProps> = ({ i }) => {
  return (
    <div className="w-[100vw] min-h-[75vh] flex flex-col-reverse lg:flex-row items-center justify-center gap-20 lg:px-40 md:px-10 px-10 snap snap-center">
      <div className="lg:w-7/12 w-12/12">
        <div className="lg:flex gap-2 items-center hidden">
        ...
        <div className="pt-2 text-center lg:text-left">
          <h1 className="xl:text-7xl text-4xl font-black">{founders[i].name}</h1>
          <p className="xl:text-2xl pt-5 font-extralight">{founders[i].quote}</p>
        </div>
      </div>
      <div className="lg:w-3/12">
        <Image
